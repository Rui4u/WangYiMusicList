...
/** 每一列之间的间距*/
static const NSInteger WSLDefaultColumeMargin = 3;
/** 每一行之间的间距*/
static const CGFloat WSLDefaultRowMargin = 3;
/** 边缘之间的间距*/
...
- (CGRect)itemFrameOfVerticalWaterFlow:(NSIndexPath *)indexPath{
    ...
    NSInteger  tempCount = self.columnCount;
#warning 写死
    ...
    CGFloat collectionW = self.collectionView.frame.size.width;
    //设置布局属性item的frame
    CGFloat w = (collectionW - self.edgeInsets.left - self.edgeInsets.right - (tempCount - 1) * self.columnMargin) / tempCount;
    CGFloat h = [self.delegate waterFlowLayout:self heightForItemAtIndexPath:indexPath itemWidth:w];
    
    //找出高度最短的那一列
    NSInteger destColumn = 0;
    CGFloat minColumnHeight = [self.columnHeights[0] doubleValue];
    for (NSInteger i = 1; i <tempCount; i++) {
        //取出第i列
